...
class IDSchema:
    ...
    def generate_n_unique(cls, n: int, max_retry: int = 20) -> List[str]:
        ...
        while len(ids) < n:
            ...
            if new_id in ids:
                ...
                if max_retry == 0:
                    raise RuntimeError(
                        f"Max Retry exceeded trying to generate {n!s} unique ids."
                    )
                continue
            ids.append(new_id)
        return ids
    ...
    def validate(cls, id_: str, max_reductions: int = 5000, iterative=True) -> bool:
        ...
        if not iterative and (rec_limit := sys.getrecursionlimit()) <= len(
            cls.Components
        ):
            raise ValueError(
                f"Number of Components must be smaller than recursion limit ({rec_limit!s})"
            )
        if iterative:
    ...
    def _validate_recursive(
        cls, id_: str, cur_idx: int = 0, max_reductions: int = 5000
    ) -> bool:
        if max_reductions == 0:
        ...
        for sub_id in sub_ids:
            success, max_reductions = cls._validate_recursive(
                sub_id, cur_idx + 1, max_reductions - 1
            )
            if success:
